...
coarse_subdecision_map_dir = Path("data/json/coarse_decision_type.json")

df = pd.read_csv(ROOT_PATH / "data" / true_df_path)
fine_sd_df = df[["file_name", "subdecision_coarse"]].copy()

fine_labels = json.loads((ROOT_PATH / coarse_subdecision_map_dir).read_text(encoding="utf-8")).keys()
...
def evaluate_model(model_name: str, model_path: Path) -> dict | None:
    ...
    y_true = merged["subdecision_coarse_true"].astype(int).to_numpy()
...
save_dir = ROOT_PATH / "data/csv/evaluate_result/subdecision_coarse"
save_dir.mkdir(parents=True, exist_ok=True)
ts = datetime.now().strftime("%Y%m%d_%H%M%S")
save_path = save_dir / f"subdecision_coarse_eval_{ts}.csv"
