...
async def predict_board_ruling(path, system_prompt, base_prompt, client, output_dir, model, stats, lock):
...
def main(args):
    ...
    async def sem_task(path):
        async with sem:
            await predict_board_ruling(path, system_prompt, base_prompt, client, output_dir, model, stats, lock)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", choices=["gpt", "gpt-o", "claude", "gemini", "llama", "qwen", "solar", "mistral", "deepseek", "t5"], required=False, default="gpt", help="LLM Model for board ruling prediction")
